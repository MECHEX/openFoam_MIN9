...
# Prepare V4b_3D run004c: controlled Lout=16D outlet-sensitivity case.
#
# This intentionally follows run004b: level-2 hot-tube surface refinement,
# BL layers on tube/fins, and the same short level-1 wake refinement. The
# extra length is a coarse outlet buffer so the comparison isolates outlet
# distance rather than changing the near-wake mesh strategy.

SRC="${SRC:-/home/hexmachina/of_runs/V4b_3D_run004b}"
DST="${DST:-/home/hexmachina/of_runs/V4b_3D_run004c}"
RUN_ID="${RUN_ID:-run004c}"
NPROCS="${NPROCS:-20}"
END_TIME="${END_TIME:-6}"
...
- Parent case: ${SRC}
- Purpose: Lout=16D check against run004b Lout=8D before accepting outlet independence
- Re: 200
- Lin: 2D
- Lout: 16D
- Lx: 243.71 mm
