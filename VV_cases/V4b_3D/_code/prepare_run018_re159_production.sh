...
# Prepare a production-geometry Re=159 V4b case with the same sampling contract
# as run017/run015, for Hopf-onset bracketing between Re=158 and Re=160.

SRC="${SRC:-/home/hexmachina/of_runs/V4b_3D_run008}"
DST="${DST:-/home/hexmachina/of_runs/V4b_3D_run018_re159_production}"
RUN_ID="${RUN_ID:-run018_re159}"
NPROCS="${NPROCS:-20}"
END_TIME="${END_TIME:-10}"
MAX_CO="${MAX_CO:-0.8}"
U_INF="${U_INF:-0.2008647}"
...
- Parent case: ${SRC}
- Purpose: production-geometry Re=159 midpoint for Hopf-onset bracketing
...
- U_inf: ${U_INF} m/s
- Re: 159, using D=0.012 m and the same viscosity basis as the V4b campaign
- maxCo: ${MAX_CO}
...
- Re=158 reached a steady late-time state.
- Re=160 showed growing/persistent late-window oscillations.
- If late-window Cl_std remains finite or grows with a spectral peak, Re=159 is post-Hopf/periodic.
- If late-window Cl_std decays toward numerical noise, Re=159 is pre-Hopf/steady.
- If the signal is marginal, the current critical Reynolds number bracket remains Re=159-160 or Re=158-159 depending on late-window behavior.
EOF
