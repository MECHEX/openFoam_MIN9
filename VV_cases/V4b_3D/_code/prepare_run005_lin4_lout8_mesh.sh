...
# Prepare V4b_3D run005: controlled inlet-sensitivity case.
#
# This follows the accepted run004b production-domain family but extends only
# the upstream inlet from Lin=2D to Lin=4D. Lout remains 8D, and the hot-wall
# refinement, boundary-layer request, and near-wake refinement are kept matched
# to run004b so the comparison isolates inlet distance.

SRC="${SRC:-/home/hexmachina/of_runs/V4b_3D_run004b}"
DST="${DST:-/home/hexmachina/of_runs/V4b_3D_run005}"
RUN_ID="${RUN_ID:-run005}"
NPROCS="${NPROCS:-20}"
END_TIME="${END_TIME:-6}"
...
(
    (${X_MIN}     -0.016 -0.006)
    (${X_FIN_MIN} -0.016 -0.006)
    (${X_FIN_MAX} -0.016 -0.006)
    (${X_OUT}     -0.016 -0.006)
    (${X_MIN}      0.016 -0.006)
    (${X_FIN_MIN}  0.016 -0.006)
    (${X_FIN_MAX}  0.016 -0.006)
...
blocks
(
    hex (0 1 5 4 8 9 13 12)   (${INLET_CELLS} 32 18) simpleGrading (1 1 1)
    hex (1 2 6 5 9 10 14 13)  (${FIN_CELLS} 32 18) simpleGrading (1 1 1)
    hex (2 3 7 6 10 11 15 14) (${OUTLET_CELLS} 32 18) simpleGrading (1 1 1)
);
...
- Parent case: ${SRC}
- Purpose: inlet-sensitivity check after outlet independence closure
- Re: 200
- Lin: 4D
- Lout: 8D
- Lx: 171.71 mm
...
- Wake refinement: same short level-1 box as run004b, x = 0..72 mm, y = +/-12 mm
- Isolated change vs run004b: upstream inlet buffer from 2D to 4D
EOF
