...
# Prepare a production-geometry Re=155 V4b case with the same sampling contract
# as run008/run012-run015, for Hopf-onset bracketing.

SRC="${SRC:-/home/hexmachina/of_runs/V4b_3D_run008}"
DST="${DST:-/home/hexmachina/of_runs/V4b_3D_run016_re155_production}"
RUN_ID="${RUN_ID:-run016_re155}"
NPROCS="${NPROCS:-20}"
END_TIME="${END_TIME:-10}"
MAX_CO="${MAX_CO:-0.8}"
U_INF="${U_INF:-0.1958115}"
...
- Parent case: ${SRC}
- Purpose: production-geometry Re=155 point for Hopf-onset bracketing
...
- U_inf: ${U_INF} m/s
- Re: 155, using D=0.012 m and the same viscosity basis as the V4b campaign
- maxCo: ${MAX_CO}
...
- If late-window Cl_std remains finite or grows with a spectral peak, Re=155 is post-Hopf/periodic.
- If late-window Cl_std decays toward numerical noise, Re=155 is pre-Hopf/steady.
- If the signal is marginal, continue bracketing with Re=152.5 or Re=157.5.
EOF
