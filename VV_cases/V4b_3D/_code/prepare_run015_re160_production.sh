...
# Prepare a production-geometry Re=160 V4b case with the same sampling contract
# as run008/run012-run014, for Hopf-onset bracketing.

SRC="${SRC:-/home/hexmachina/of_runs/V4b_3D_run008}"
DST="${DST:-/home/hexmachina/of_runs/V4b_3D_run015_re160_production}"
RUN_ID="${RUN_ID:-run015_re160}"
NPROCS="${NPROCS:-20}"
END_TIME="${END_TIME:-10}"
MAX_CO="${MAX_CO:-0.8}"
U_INF="${U_INF:-0.202128}"
...
- Parent case: ${SRC}
- Purpose: production-geometry Re=160 point for Hopf-onset bracketing
...
- U_inf: ${U_INF} m/s
- Re: 160, using D=0.012 m and the same viscosity basis as the V4b campaign
- maxCo: ${MAX_CO}
...
- Re=150 decayed to a steady late-time state.
- Re=175 showed persistent late-window oscillations.
- If late-window Cl_std remains finite with a robust spectral peak, Re=160 is post-Hopf/periodic.
- If late-window Cl_std decays toward numerical noise, Re=160 is pre-Hopf/steady.
- If the signal is weakly growing or weakly decaying, continue bracketing with Re=165 or Re=155.
EOF
