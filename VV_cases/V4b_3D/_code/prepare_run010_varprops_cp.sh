...
# Prepare V4b_3D run010: run008-style production rerun with variable air
# properties, Cp-like heat capacity, and dense full-field output.

SRC="${SRC:-/home/hexmachina/of_runs/V4b_3D_run008}"
DST="${DST:-/home/hexmachina/of_runs/V4b_3D_run010_varprops_cp}"
RUN_ID="${RUN_ID:-run010_varprops_cp}"
NPROCS="${NPROCS:-20}"
...
- Parent case: ${SRC}
- Purpose: run008-style rerun with variable air properties, Cp-like capacity,
  and dense movie-ready output
...
- Thermodynamics: eConst, sensibleInternalEnergy, Cv=1005 J/(kg K)
- Naming note: this is called cp because the energy capacity is set to the
  same value used for Cp in the accepted run008 thermal scaling, while retaining
  the stable eConst/internal-energy formulation.
