"""Very early t=0.2 smoke comparison for run004b vs run007c.

This is intentionally a startup sanity check, not a production averaging
window. It compares force statistics over 0.1..0.2 s and thermal quantities at
the reconstructed t=0.2 checkpoint.
"""
...
RUNS = {
    "run004b": {
        ...
        "capacity": 718.0,
        "label": "baseline eConst Cv=718",
    },
    "run007c": {
        "case": Path("/home/hexmachina/of_runs/V4b_3D_run007c"),
        "capacity": 1005.0,
        "label": "eConst Cp-scale capacity=1005",
    },
}
...
def outlet_stats(case: Path, capacity: float) -> dict[str, float]:
    ...
    q_air_case_capacity = m_dot * capacity * (t_mass - T_IN)
    q_air_cp1005 = m_dot * 1005.0 * (t_mass - T_IN)
    return {
        "T_out_area_K": t_area,
        "T_out_mass_K": t_mass,
        "m_dot_proxy": m_dot,
        "Q_air_case_capacity_W": q_air_case_capacity,
        "Q_air_Cp1005_W": q_air_cp1005,
    }
...
def thermal_derived(row: dict[str, float], capacity: float) -> None:
    k = MU_AIR * capacity / PR_AIR
    h_wall = row["Q_wall_hot_total_W"] / (A_HOT_TOTAL * lmtd(row["T_out_mass_K"]))
    row["k_case_W_mK"] = k
    row["Nu_wall_case_k"] = h_wall * D / k
    row["wall_vs_air_case_capacity_pct"] = pct(row["Q_wall_hot_total_W"], row["Q_air_case_capacity_W"])
    row["wall_vs_air_Cp1005_pct"] = pct(row["Q_wall_hot_total_W"], row["Q_air_Cp1005_W"])
...
def main() -> None:
    ...
    for run, meta in RUNS.items():
        row = {
            ...
            "capacity_used_in_case": meta["capacity"],
        }
        row.update(force_stats(meta["case"]))
        row.update(outlet_stats(meta["case"], meta["capacity"]))
        row.update(wall_heat_flux(meta["case"]))
        thermal_derived(row, meta["capacity"])
        rows.append(row)
    ...
    for row in rows:
        for key in (
            ...
            "T_out_mass_K",
            "Q_wall_hot_total_W",
            "Q_air_case_capacity_W",
            "Q_air_Cp1005_W",
            "Nu_wall_case_k",
        ):
            row[f"{key}_diff_pct_vs_run004b"] = pct(row[key], baseline[key])

    csv_path = SCRIPT_DIR / "run004b_vs_run007c_t02_quick_compare.csv"
    json_path = SCRIPT_DIR / "run004b_vs_run007c_t02_quick_compare.json"
    md_path = SCRIPT_DIR / "run004b_vs_run007c_t02_quick_compare.md"
    ...
    for row in rows:
        md.append(
            f"| {row['run']} | {f(row['capacity_used_in_case'], 0)} | "
            f"{f(row['Cd_mean_0p1_0p2'])} | {f(row['Cl_rms_0p1_0p2'])} | "
            f"{f(row['T_out_mass_K'])} | {f(row['Q_wall_hot_total_W'])} | "
            f"{f(row['Q_air_case_capacity_W'])} | {f(row['Q_air_Cp1005_W'])} | "
            f"{f(row['Nu_wall_case_k'])} |"
        )
    r = rows[1]
    md.extend(
        [
            ...
            "",
            f"- `run007c` wall heat input is {r['Q_wall_hot_total_W_diff_pct_vs_run004b']:+.2f}% vs `run004b`.",
            f"- `run007c` case-normalized wall Nu is {r['Nu_wall_case_k_diff_pct_vs_run004b']:+.2f}% vs `run004b`.",
            f"- `run007c` air-side heat pickup using its case capacity is {r['Q_air_case_capacity_W_diff_pct_vs_run004b']:+.2f}% vs `run004b`.",
            "",
            "At this very early time, the larger heat flux mainly follows the increased thermal conductivity/capacity scale. "
            "When Nu is normalized with the matching case conductivity, it is not comparably inflated.",
        ]
    )
    md_path.write_text("\n".join(md) + "\n", encoding="utf-8")
