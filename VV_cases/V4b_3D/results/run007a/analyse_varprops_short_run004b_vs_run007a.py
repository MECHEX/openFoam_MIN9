"""Short variable-property comparison for V4b_3D Re=200.

Compares run007a (incompressiblePerfectGas + Sutherland, t=2 s) against the
accepted constant-property/Boussinesq run004b over matched early windows.
"""
...
MU_AIR = 1.827e-05
PR_AIR = 0.713
K_AIR_BASELINE = CP_AIR * MU_AIR / PR_AIR

CASE_DIRS = {
    "run004b": Path("/home/hexmachina/of_runs/V4b_3D_run004b"),
    "run007a": Path("/home/hexmachina/of_runs/V4b_3D_run007a"),
}
...
class Row:
    ...
    Q_diff_pct_vs_run004b: float | None = None
...
def thermal_stats(case_dir: Path, start: float, end: float) -> dict[str, float]:
    ...
    for time_name in times:
        ...
        nu = (q / (A_HOT_TOTAL * lmtd(t_area))) * D / K_AIR_BASELINE
...
def build_rows() -> list[Row]:
    ...
    for start, end in WINDOWS:
        ...
        for run, case_dir in CASE_DIRS.items():
            ...
            row = Row(
                run=run,
                model="Boussinesq_const" if run == "run004b" else "incompressiblePerfectGas_sutherland",
            ...
            if run == "run004b":
                ...
                row.Q_diff_pct_vs_run004b = pct(row.Q_total_W, baseline.Q_total_W)
...
def write_outputs(rows: list[Row]) -> None:
    with (SCRIPT_DIR / "run004b_vs_run007a_varprops_short_compare.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(asdict(rows[0]).keys()))
        writer.writeheader()
        for row in rows:
            writer.writerow(asdict(row))
    (SCRIPT_DIR / "run004b_vs_run007a_varprops_short_compare.json").write_text(
...
def write_markdown(rows: list[Row]) -> None:
    ...
    for row in primary:
        lines.append(
            f"| {row.run} | {row.model} | {row.window} | {fmt(row.Cd_mean, 6)} | "
    ...
    lines += [
        ...
        "| Quantity | Difference |",
        "|---|---:|",
        f"| Cd_mean | {fmt_pct(run007.Cd_diff_pct_vs_run004b)} |",
        f"| Cl_rms | {fmt_pct(run007.Cl_rms_diff_pct_vs_run004b)} |",
        f"| St | {fmt_pct(run007.St_diff_pct_vs_run004b)} |",
        f"| T_out | {run007.T_out_diff_K_vs_run004b:+.3f} K |",
        f"| Q_total | {fmt_pct(run007.Q_diff_pct_vs_run004b)} |",
        f"| Nu_EB | {fmt_pct(run007.Nu_diff_pct_vs_run004b)} |",
        "",
    ...
    (SCRIPT_DIR / "run004b_vs_run007a_varprops_short_compare.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    for row in rows:
        if row.run == "run007a":
            print(
                f"{row.window}: Cd diff={row.Cd_diff_pct_vs_run004b:.3f}%, "
                f"St diff={row.St_diff_pct_vs_run004b:.3f}%, "
                f"T_out diff={row.T_out_diff_K_vs_run004b:+.3f} K, "
                f"Nu diff={row.Nu_diff_pct_vs_run004b:.3f}%"
            )
