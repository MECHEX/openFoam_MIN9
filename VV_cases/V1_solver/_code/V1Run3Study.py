...
RUN_ROOT = Path(r"C:\openfoam-case\VV_cases\V1_solver_run003")
RESULTS_ROOT = REPO_CASE / "results" / "study_v1"
RUNS_RESULTS_ROOT = RESULTS_ROOT / "runs"
ACTIVE_RUN_NAME = "003_data_sensitivity_fine_and_long"
...
# fine:  base 2 mm, level 4 on surface (0.125 mm), level 2 near (0.5 mm),
#        level 2 wake (0.5 mm), 8 BL layers on cylinder, 3 on walls → ~100 k cells
# long:  identical to run002 medium but downstream extended to 40D
MESHES = {
    "fine": MeshVariant("fine", 0.002, 4, 2, 2, 8, 3, 0.25, 1.15),
    "long": MeshVariant("long", 0.0025, 3, 2, 1, 6, 2, 0.25, 1.20),
}

CASES = [
    StudyCase("b050_fine_Re130", "beta=0.50 fine-mesh onset lower bound",  0.50, 130.0, UP_D, DOWN_D_MEDIUM, MESHES["fine"], 15.0),
    StudyCase("b050_fine_Re135", "beta=0.50 fine-mesh onset upper bound",  0.50, 135.0, UP_D, DOWN_D_MEDIUM, MESHES["fine"], 15.0),
    StudyCase("b050_long_Re130", "beta=0.50 long-domain onset lower bound", 0.50, 130.0, UP_D, DOWN_D_LONG,   MESHES["long"], 15.0),
    StudyCase("b050_long_Re135", "beta=0.50 long-domain onset upper bound", 0.50, 135.0, UP_D, DOWN_D_LONG,   MESHES["long"], 15.0),
]
...
NP = 4  # MPI ranks for parallel pimpleFoam
...
def allrun_file() -> str:
    return (
        ...
        f'decomposePar | tee logs/decomposePar.log\n'
        f'mpirun -np {NP} pimpleFoam -parallel | tee logs/pimpleFoam.log\n'
...
def write_run_notes() -> None:
    lines = [
        ...
        "## Purpose",
        "",
        "Investigate whether the bracket Re ∈ (130, 135) for β=0.50 onset is affected by:",
        "1. Mesh resolution (fine: ~100 k cells vs medium: ~38 k cells)",
        "2. Outlet domain length (long: 40D vs medium: 20D)",
        "",
        "## Reference",
        "",
        f"- Sahin & Owens (2004): Re_crit = 124.09 for β=0.50",
        f"- V1 run 002 medium mesh bracket: Re ∈ (130, 135)",
        "",
        "## Case matrix",
        "",
        "| case | mesh | downstream_D | Re | end_time_s |",
        "|---|---|---:|---:|---:|",
    ]
    for case in CASES:
        lines.append(f"| {case.name} | {case.mesh.name} | {case.downstream_D:.0f} | {case.reynolds:.0f} | {case.end_time_s:.0f} |")
    write_text(ACTIVE_RUN_NOTES_DIR / "run_scope.md", "\n".join(lines) + "\n", encoding="utf-8")
...
def write_case(case: StudyCase, overwrite: bool) -> None:
    ...
    if case_dir.exists():
        if not overwrite:
            ...
            return
        shutil.rmtree(case_dir, onexc=_rm_readonly)
    ...
    write_text(case_dir / "system" / "decomposeParDict", decompose_par_dict())
...
def write_output_md(case: StudyCase, mesh_info: dict[str, object], metrics: dict[str, object]) -> None:
    ...
    if metrics["St"] is not None and ref["St_crit"] not in (None, 0):
        delta_st = round(100.0 * (float(metrics["St"]) - float(ref["St_crit"])) / float(ref["St_crit"]), 3)
    lines = [
        f"# {case.name} output",
        "",
...
def write_summary(rows: list[dict[str, object]]) -> None:
    ...
    for target in (ACTIVE_RUN_SUMMARY_DIR, STUDY_SUMMARY_DIR):
        ...
        lines = [
            "# V1 run 003 summary",
            "",
            "| case | mesh | downstream_D | Re | cells | regime | St_sim | St_ref | dSt [%] |",
            "|---|---|---:|---:|---:|---|---:|---:|---:|",
        ]
        for row in rows:
            lines.append(f"| {row['case']} | {row['mesh']} | {row['downstream_D']} | {row['Re']} | {row['cells']} | {row['regime']} | {row['St_sim']} | {row['so_St_crit']} | {row['delta_St_percent']} |")
...
def analyze(names: list[str]) -> None:
    ...
    for case in resolve_cases(names):
        ...
        row = {
            ...
            "Cl_rms": metrics["Cl_rms"],
            "frequency_hz": metrics["frequency_hz"],
            "St_sim": metrics["St"],
            "so_Re_crit": ref["Re_crit"],
            "so_St_crit": ref["St_crit"],
            "delta_St_percent": delta,
            "status": metrics["status"],
        }
        rows.append(row)
        write_text(
            sim_dir(case.name, create=True) / "03_processed_data" / "summary.json",
            json.dumps({"metadata": metadata(case), "mesh": mesh_info, "metrics": metrics, "comparison": row}, indent=2) + "\n",
            encoding="utf-8",
        )
...
def main() -> None:
    args = sys.argv[1:]
    if not args:
        print("Usage: python _code/V1Run3Study.py prepare|analyze [case_name] [--overwrite]")
        return
    cmd = args[0]
